...
import { BiHome, BiMessageDetail, BiDollarCircle } from "react-icons/bi";
  ...
  return (
    <nav className="flex px-10 py-2  bg-gray-100 items-center border-b-4 border-white shadow z-20 fixed w-full">
      ...
      <div className="relative">
        <BiUser className="text-3xl cursor-pointer" onClick={toggleDropdownVisible} />
        {dropdownVisible && (
          <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10">
            <div className="py-1">
              <p className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Username: {user.name}</p>
              <p className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Profile</p>
              <p className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Edit Profile <BiEdit className="inline-block ml-1" /></p>
              {user && user.name ? (
        <button
          onClick={(e) => {
            e.preventDefault();
